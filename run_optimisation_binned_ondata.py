...
df_realdata = pd.read_csv(path_to_dir+'radial_migration_kernel/data/LAMOST_Gaia_subgiants_Xiangetal2024_kinematics.csv')
F = df_realdata['FEH']
L = df_realdata['Lz']
age = df_realdata['AGE']
...
final_file = path_to_dir+f'radial_migration_kernel/data/L@{Rg_l}_{Rg_r}/minimization_result_{Nknots}knots_L@{Rg_l}to{Rg_r}.npy'#_L@10
...
aux_params = {'MH_at_8_0': 0.04, 'ln_MH_grad_0': -2.66, 'tol': 5e-2}  # Auxiliary parameters for the model
...
# ]))
x0 = jnp.array(jnp.concatenate([
    jnp.array(np.random.uniform(-0.2, 1., Nknots)),  # ln_Rdisk knots
    jnp.array(np.random.uniform(4, 5, Nknots)),  # ln_sigmaLz knots
    jnp.array(np.linspace(0.1, -1.5, Nknots)),
